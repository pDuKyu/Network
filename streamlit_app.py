...
text2 = """
**라우터**는 패킷을 읽고 목적지 IP 주소를 기반으로 최적의 경로를 결정합니다. 그리고 그 경로에 따라 다음 라우터로 패킷을 전송합니다. 
...
라우터는 이러한 패킷을 읽고 목적지 IP 주소를 기반으로 최적의 경로를 결정합니다. 그리고 그 경로에 따라 다음 라우터로 패킷을 전송합니다. 이러한 과정을 통해 데이터가 출발지에서 목적지로 안전하고 효율적으로 전달됩니다.

간단히 말해, 라우터는 네트워크 간에 데이터를 보내고 받는데 사용되며, 각 패킷의 목적지를 확인하여 최적의 경로를 선택하여 데이터를 전송합니다.ㅋ
